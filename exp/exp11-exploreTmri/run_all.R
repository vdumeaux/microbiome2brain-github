...
dat0 <- read.csv("/home/data/refined/microbiome2brain/MRI/T_results.txt", sep = "\t", row.names = 1)
...
pheatmap::pheatmap(dat1, scale = "row", annotation_col = cl[, !colnames(cl) %in% ("id")])
...
p000 <- ggplot(long.dat, aes(x=Tvalue))+
  ...
  ggtitle("Tvalue all contrasts, all ROI per participant and visit")
...
p0001 <- ggplot(long.dat, aes(x=Tvalue))+
  ...
  geom_vline(xintercept = c(-2.3, 2.3), color = "red", linetype = "dashed") +
  geom_vline(xintercept = 0, color = "red") +
  ggtitle("Tvalue aall ROI per participant, per contrast and visit")
...
pdf("output/hist_Tvalues_participant_visit.pdf", width = 15, height = 10)
plot.lists
dev.off()
